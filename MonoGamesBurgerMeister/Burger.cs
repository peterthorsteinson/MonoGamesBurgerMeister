...
namespace GameProject
{
    ...
    public class Burger
    {
        ...
        /// <param name="keyboardState">the current state of the keyboardS</param>
        public void Update(GameTime gameTime, KeyboardState keyboardState) 
        {
            // burger should only respond to input if it still has health
            if (health > 0)
            {
                // move burger using keyboard WASD for movement and spacebar for shooting fries
                if (keyboardState.IsKeyDown(Keys.W))
                {
                    drawRectangle.Y -= GameConstants.BURGER_MOVEMENT_AMOUNT; //move burger up
                }
                if (keyboardState.IsKeyDown(Keys.A))
                {
                    drawRectangle.X -= GameConstants.BURGER_MOVEMENT_AMOUNT; //move burger left
                }
                if (keyboardState.IsKeyDown(Keys.S))
                {
                    drawRectangle.Y += GameConstants.BURGER_MOVEMENT_AMOUNT; //move burger down
                }
                if (keyboardState.IsKeyDown(Keys.D))
                {
                    drawRectangle.X += GameConstants.BURGER_MOVEMENT_AMOUNT; //move burger right
                }
                ...
                if (!canShoot)
                {
                    ...
                    if (elapsedCooldownTime >= GameConstants.BURGER_COOLDOWN_MILLISECONDS || keyboardState.IsKeyUp(Keys.Space))
                    {
                        canShoot = true;
                ...
                if (health > 0 && keyboardState.IsKeyDown(Keys.Space) && canShoot)
                {
                    canShoot = false;
