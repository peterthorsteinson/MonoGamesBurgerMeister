...
namespace GameProject
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        // health support
        string healthString = GameConstants.HEALTH_PREFIX + GameConstants.BURGER_INITIAL_HEALTH;
        ...
        protected override void LoadContent()
        {
            ...
            //The following do NotFiniteNumberException work when using monogames and XNA on Visual Studio2013:
            ...
            using (var stream = TitleContainer.OpenStream(Path.Combine(Content.RootDirectory, "sounds/BurgerDamage.wav")))
            {
                burgerDamage = SoundEffect.FromStream(stream);
            }
            using (var stream = TitleContainer.OpenStream(Path.Combine(Content.RootDirectory, "sounds/BurgerDeath.wav")))
            {
                burgerDeath = SoundEffect.FromStream(stream);
            }
            using (var stream = TitleContainer.OpenStream(Path.Combine(Content.RootDirectory, "sounds/BurgerShot.wav")))
            {
                burgerShot = SoundEffect.FromStream(stream);
            }
            using (var stream = TitleContainer.OpenStream(Path.Combine(Content.RootDirectory, "sounds/Explosion.wav")))
            {
                explosion = SoundEffect.FromStream(stream);
            }
            using (var stream = TitleContainer.OpenStream(Path.Combine(Content.RootDirectory, "sounds/TeddyBounce.wav")))
            {
                teddyBounce = SoundEffect.FromStream(stream);
            }
            using (var stream = TitleContainer.OpenStream(Path.Combine(Content.RootDirectory, "sounds/TeddyShot.wav")))
            {
                teddyShot = SoundEffect.FromStream(stream);
            }
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            foreach (TeddyBear b in bears)
            {
                if (b.Active && b.CollisionRectangle.Intersects(burger.CollisionRectangle))
                {
                    burger.Health -= GameConstants.BEAR_DAMAGE;
                    b.Active = false;
                    explosions.Add(new Explosion(explosionSpriteStrip, b.Location.X, b.Location.Y, explosion));
                    burgerDamage.Play();
                    CheckBurgerKill();
                    healthString = GameConstants.HEALTH_PREFIX + burger.Health;
                }
            ...
            foreach (Projectile projectile in projectiles)
            {
                ...
                {                    
                    ...
                    explosions.Add(new Explosion(explosionSpriteStrip, projectile.CollisionRectangle.Center.X, projectile.CollisionRectangle.Center.Y, explosion));
                    burgerDamage.Play();
                    CheckBurgerKill();
                    healthString = GameConstants.HEALTH_PREFIX + burger.Health;
                }
            ...
            // spawn new teddy bears until we have GameConstants.MAX_BEARS bears again
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            spriteBatch.DrawString(font, healthString, GameConstants.HEALTH_LOCATION, Color.Red);
            spriteBatch.DrawString(font, scoreString, GameConstants.SCORE_LOCATION, Color.Red);
